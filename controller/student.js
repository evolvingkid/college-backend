...
exports.bulkStudentCreation = async (req, res) => {
    ...
    if (usertype === 'Student') {
        ...
        userData = await studentCreationBuilkData(req);
    }
...
async function studentCreationBuilkData(req) {
    ...
    if (isBatchAlready) {
        await Promise.all([User.insertMany(bulkuser), Studnet.insertMany(bulkStudent)])
        return bulkuser;
    }
