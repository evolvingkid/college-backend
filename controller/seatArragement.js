...
exports.seatArragemnet = async (req, res) => {
    ...
    for (let index = 0; index < courseData.length; index++) {
        batches.push({ id: courseData[index].activeExam[0].batch, course: courseData[index]._id });
    }
    ...
    for (let index = 0; index < batches.length; index++) {
        ...
        for (let j = 0; j < studentData.length; j++) {
            ...
            student.push({ id: studentData[j]._id, batch: batches[index].id, course: batches[index].course });
    ...
    for (let index = 0; index < examHallData.length; index++) {
        ...
        usedHall.push({ examHallID: false, splitStergnth: stength, splitLegth: 2, id: examHallData[index]._id });
    }

    if (examCount < student.length) {
        return res.status(400).json({ msg: "Their is no enough seat for all students" });
    }
    ...
    for (let index = 0; index < student.length; index++) {
        ...
        if (usedHall[currentExamhall].splitStergnth == 0) {
            ...
            if (usedHall[currentExamhall].splitLegth !== 0) {
                usedHall[currentExamhall].splitLegth == usedHall[currentExamhall].splitLegth - 1
            } else {
                currentExamhall++;
                index--;
                continue;
            ...
            usedHall[currentExamhall].splitStergnth = parseInt(examHallData[currentExamhall].usedCount / usedHall[currentExamhall].splitLegth)
            currentExamhall++;
        ...
        usedHall[currentExamhall].splitStergnth = usedHall[currentExamhall].splitStergnth - 1;
